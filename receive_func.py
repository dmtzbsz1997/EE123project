...
def recon(data, uprate = 5):
    ...
    def upsample(img,odim):
        return [scipy.misc.imresize(i, odim, interp = 'bilinear', mode = 'RGB') for i in img]
...
class TNCaprs:
    ...
    def __init__(self, fs = 48000.0, Abuffer = 512, Nchunks=10,baud=1200,center=1700):
...
#         self.h_lpp = signal.firwin(self.N,self.BW*2*3/self.fs,window='hanning')
...
#         self.h_bp = (signal.firwin(self.N,self.BW/self.fs*5,window='hanning'))*exp(1j*2*pi*self.center_f*r_[-self.N/2:self.N/2]/self.fs)
...
#         freq = self.center_f + 1000*NRZ.ravel()
